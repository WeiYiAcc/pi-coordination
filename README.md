...
**Dashboard Sections:**
- **Pipeline**: Phase progress (scout -> planner -> workers -> review -> complete)
- **Task Queue**: Tasks with status, dependencies, worker assignments
- **Workers**: Scrollable grid with status, duration, cost, turns, progress
- **Events**: Recent activity stream
- **Cost Breakdown**: Spend by phase and by worker
